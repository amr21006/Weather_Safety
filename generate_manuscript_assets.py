...
construction safety manuscript, with EVERY value computed from the real,
leakage-free model (no hard-coded numbers).

Implements the methodology described in the manuscript:
  * leakage-free employer historical severity (past incidents only)
  * weather reconstruction -> PCA orthogonalisation (weather_pc1, weather_pc2)
  * rule-based 15-class NLP equipment ontology
  * temporal validation split (cut-off 2023-04-30) + SMOTE within training
  * 16-algorithm benchmark with bootstrap 95% CIs
  * calibration, precision-recall, ablation/sensitivity, permutation importance
...
# Words scrubbed from narratives BEFORE text vectorisation, to remove explicit
# outcome / treatment leakage. Deliberately broad (extends the original list).
SCRUB = (r"hospital|admit|admitted|surg|amputat|amputee|clinic|\ber\b|emergency|"
         r"medical|medevac|life[\s-]?flight|ambulance|paramedic|icu|in[\s-]?patient|"
         r"transport(ed)?|treat|treated|treatment|fatal|fatality|deceased|dead|died|"
         r"death|killed|injur|wound|prognosis|recover")
...
def build_features(df, train_mask):
    ...
    scaler_w = StandardScaler().fit(df.loc[train_mask, WEATHER_RAW].fillna(df[WEATHER_RAW].median()))
    Wz = scaler_w.transform(df[WEATHER_RAW].fillna(df[WEATHER_RAW].median()))
...
def figure5_cv(cv_results):
    ...
    ax.set_xlabel("ROC AUC  (Stratified 5-Fold CV, SMOTE applied within each fold)")
    ax.grid(axis="y", visible=False); _style_ax(ax)
    _finish(fig, "Figure_5_CV_Performance.png",
            f"Figure 5. Distribution of ROC AUC across stratified 5-fold cross-validation for all {len(names)} algorithms.",
            f"SMOTE is applied within each training fold only; the focal {FOCAL} is highlighted. "
...
def reviewer_confusion(df):
    """NLP validation confusion matrix. If 'nlp_manual_annotations.csv'
    (columns: manual_label, nlp_label) exists, draw the REAL matrix. Otherwise
    emit a ready-to-fill template for the 100-narrative validation subset (the
    figure itself is NOT fabricated)."""
    path = os.path.join(DATA_DIR, "nlp_manual_annotations.csv")
    if not os.path.exists(path):
        sample = df.sample(n=min(100, len(df)), random_state=SEED).copy()
        tmpl = pd.DataFrame({
            "row_id": sample.index,
            "narrative_excerpt": sample["Final Narrative"].fillna("").str.slice(0, 240),
            "nlp_label": sample["nlp_equipment"].astype(str),
            "manual_label": "",   # <-- research team fills this in
        })
        tmpl_path = os.path.join(DATA_DIR, "nlp_manual_annotations_TEMPLATE.csv")
        tmpl.to_csv(tmpl_path, index=False, encoding="utf-8")
        print("  [Reviewer Fig 1] No annotations yet -> wrote "
              "'nlp_manual_annotations_TEMPLATE.csv' (100 narratives). Fill 'manual_label', "
              "save as 'nlp_manual_annotations.csv', re-run -> real confusion matrix. (Nothing fabricated.)")
        return
    from sklearn.metrics import confusion_matrix
    ann = pd.read_csv(path)
    labels = sorted(set(ann["manual_label"].astype(str)) | set(ann["nlp_label"].astype(str)))
    cm = confusion_matrix(ann["manual_label"].astype(str), ann["nlp_label"].astype(str), labels=labels)
    ...
    for (i, j), z in np.ndenumerate(cm):
        ax.text(j, i, str(int(z)), ha="center", va="center",
                color="white" if z > cm.max() / 2 else "black", fontsize=9)
    ax.set_xlabel("NLP-Assigned Class"); ax.set_ylabel("Manual Annotation")
    _finish(fig, "Reviewer_Fig_1_NLP_Confusion_Matrix.png",
            f"Reviewer Fig. 1. NLP ontology validation confusion matrix (exact-match accuracy = {acc:.1%}).",
            f"Manual annotation of {int(cm.sum())} narratives versus the rule-based NLP assignment.")
...
def write_tables(bench, importance_tbl, vif_tbl, gonogo_tbl, ablation, n_total, n_train, n_test):
    ...
    A(f"_Dataset: {n_total} maritime incidents (train ≤ 2023-04-30: {n_train}; "
      ...
      f"engineered features. All values computed from real model output (no hard-coding)._\n")
...
def main():
    ...
    # ---- Cross-validation (SMOTE inside folds) -------------------------------
    print("[B] 5-fold cross-validation (SMOTE within folds) ...")
    cv_results = {}
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=SEED)
    for name in bench:          # ALL trained algorithms, not just the top ones
        try:
            pipe = ImbPipeline([("smote", SMOTE(random_state=SEED)), ("clf", model_zoo()[name])])
            cv_results[name] = cross_val_score(pipe, Xtr_s, ytr, cv=skf, scoring="roc_auc")
        except Exception as e:
            print(f"    {name} CV failed: {str(e)[:60]}")
    ...
    print(f"{FOCAL} CV AUC : {np.mean(cv_results[FOCAL]):.3f} ± {np.std(cv_results[FOCAL]):.3f}   (5-fold)")
...
def build_gonogo(df, feat_cols, scaler, scaler_w, pca, Xtr, ytr, pre_cols):
    ...
    sev_q = df["employer_historical_severity"]
